...
import polyglot.{CellInGame, Pair}
...
class LogicsImpl(private val size: Int, private val mines: Int) extends Logics:
  ...
  private var board: Sequence[Cell] =
  ...
  private def randomPosition(numberOfMines: Int): Sequence[Point2D] =
    ...
    while positions.size() <= numberOfMines do
  ...
  override def won(): Boolean = board.filter(c => c.isMine && c.isRevealed).map(_.position).equals(minesPositions)
