...
object Board:
  ...
  private case class GameBoard(size: Int, mines: Int) extends Board:
    ...
    override def cellInfo(x: Int, y: Int): CellInGame =
      ...
      val neighbours = allNeighboursOf(cell.position.x, cell.position.y).filter(_.isMine).size()
...
class LogicsImpl(private val size: Int, private val mines: Int) extends Logics:
  ...
  override def hit(x: Int, y: Int): Unit =
    ...
    if !currentCell.isFlagged then
      currentCell.reveal()
      minesBoard.board.filter(c => neighbours.contains(c) && !c.isMine).map(_.reveal())

  override def flagCell(x: Int, y: Int): Unit = minesBoard.board.map(c => if c.position == Point2D(x, y) then c.flag())
  override def won(): Boolean = minesBoard.board.filter(c => !c.isMine && !c.isRevealed).size() == 0

  override def lost(): Boolean = minesBoard.board.filter(c => c.isMine && c.isRevealed).size() != 0
